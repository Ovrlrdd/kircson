<?php
    ...
    $sql = "SELECT `event`, `eventdate` FROM `cal` WHERE `event` = '$event' AND `eventdate` = '$date'";
    ...
    elseif ($rowsCount == 0){
        $mysql->query("INSERT INTO `cal` ( `title`, `event`, `eventdate`) VALUES ( '$title', '$event', '$date')");
    
        $mysql->close();
    
    header('Location: https://kircson.ru/soc_taxi.php');
    }
?>